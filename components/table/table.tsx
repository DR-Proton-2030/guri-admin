...
import { deleteBusiness, editBusiness, getFilteredBusinesses } from '../../pages/api/business';
...
export const TableWrapper = () => {
   ...
   const toggleStatus = () => {
      setStatus(prevStatus => prevStatus === 'PENDING' ? 'ACTIVE' : 'PENDING');
   };
      ...
      try {
         ...
         setBusinesses(prevBusinesses =>
            prevBusinesses.map(business =>
               business._id === id ? { ...business, status: updatedBusiness.status } : business
            )
         );
      } catch (error) {
         // Error handling
         ...
         <div style={{display:'flex',justifyContent:'space-between'}}>
            <div className="radio-inputs">
               <label className="radio">
                  <input type="radio" name="radio" onClick={() => setStatus('PENDING')} />
                  <span className="name">Pending</span>
               </label>
               <label className="radio">
                  <input type="radio" name="radio" onClick={() => setStatus('ACTIVE')} />
                        ...
                        <Table.Row key={item._id}>
                           {(columnKey) => (
                              <Table.Cell key={columnKey}>
                                 <RenderCell user={item} columnKey={columnKey} handleStatusChange={handleStatusChange} handleDelete={handleDelete} />
                              </Table.Cell>
                           )}
                        </Table.Row>
                     )}
